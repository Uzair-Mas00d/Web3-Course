...
export class ItemCancled extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ItemCancled entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ItemCancled must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("ItemCancled", id.toString(), this);
    }
  }

  static loadInBlock(id: string): ItemCancled | null {
    return changetype<ItemCancled | null>(
      store.get_in_block("ItemCancled", id),
    );
  }

  static load(id: string): ItemCancled | null {
    return changetype<ItemCancled | null>(store.get("ItemCancled", id));
  }
